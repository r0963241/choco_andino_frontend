<template>
  <div class="min-h-screen bg-[#f5fdf6] text-[#333] font-sans">
    
    <!-- 1. MASHPI-STYLE HERO IMMERSION BANNER -->
    <header class="relative w-full h-[65vh] bg-neutral-900 overflow-hidden flex items-center justify-center text-center px-4">
      <img 
        src="https://unsplash.com" 
        alt="Chocó Andino Cloud Forest Canopy" 
        class="absolute inset-0 w-full h-full object-cover opacity-55 pointer-events-none"
      />
      <div class="relative z-10 max-w-3xl fade-in-tab">
        <span class="text-xs uppercase tracking-widest font-bold text-[#f5fdf6] bg-[#2E7D32]/80 px-4 py-1.5 rounded-full inline-block mb-4 shadow">
          Metropolitan District of Quito Ecosystem
        </span>
        <h1 class="text-4xl md:text-5xl lg:text-6xl font-extrabold text-white font-['Segoe_UI'] tracking-tight mt-0 drop-shadow-md">
          A Rainforest Hotel In The Clouds
        </h1>
        <p class="text-white/90 text-base md:text-lg mt-4 max-w-2xl mx-auto drop-shadow font-['Segoe_UI']">
    ...
    <section class="max-w-4xl mx-auto px-6 pt-16 pb-8 text-center">
      <h2 class="text-3xl font-bold font-['Segoe_UI'] text-[#1B5E20] mt-0 mb-4">Regenerative Tourism & Conservation</h2>
      <p class="text-gray-600 text-base leading-relaxed font-['Segoe_UI'] max-w-3xl mx-auto">
      ...
      <div class="w-16 h-1 bg-[#2E7D32] mx-auto mt-6 rounded"></div>
    </section>

    <!-- 3. CORE MARKETPLACE DISCOVERY SECTION -->
    <main class="max-w-7xl mx-auto px-6 py-12">
      <div class="text-center mb-10">
        <h2 class="text-2xl font-bold font-['Segoe_UI'] text-[#1B5E20] mt-0">Explore Accommodations</h2>
        <p class="text-sm text-gray-500 font-['Segoe_UI'] mt-1">Tranquil sanctuaries integrating modern comforts with pristine wilderness.</p>
      </div>
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        ...
        >
          ...
          <div class="relative h-56 bg-neutral-100 overflow-hidden">
            ...
            />
            <span class="absolute top-4 left-4 text-xs font-bold uppercase tracking-wider bg-white/90 backdrop-blur-sm text-[#1B5E20] px-3 py-1 rounded shadow-sm">
              📍 {{ cabin.location }}
            </span>
          </div>
          ...
          <div class="p-6 flex flex-col flex-grow">
            <h3 class="text-xl font-bold font-['Segoe_UI'] text-[#1B5E20] mt-0 mb-2">
              {{ cabin.title }}
            </h3>
            ...
            <div class="flex items-center justify-between border-t border-gray-100 pt-4 mt-auto">
              <div>
                <span class="text-2xl font-extrabold text-[#2E7D32] tracking-tight">${{ cabin.price_per_night }}</span>
                <span class="text-xs text-gray-400 font-medium"> / night</span>
              </div>
              
              <button 
                @click="bookAccommodation(cabin.id)" 
                class="px-5 py-2.5 bg-[#1B5E20] hover:bg-[#2E7D32] text-white text-xs font-bold uppercase tracking-wider rounded-lg transition-colors duration-300 shadow-sm"
              >
                Book Stay
              </button>
            </div>
    ...
    <footer class="bg-[#1B5E20] text-white/80 text-center py-8 border-t border-green-800 text-xs tracking-wider font-['Segoe_UI']">
